...
func Test_Int_Get(t *testing.T) {
	for _, tt := range uniquerandIntTestCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			// the count must be equal to the range at this point
			if uri.Count() != uri.Range() {
				t.Errorf("Count() = %v != Range()", uri.Count())
			}
			...
			// make sure the Int state is correct
			for n := range uniqueMap {
				if gotOk := uri.Used(n); !gotOk {
					t.Errorf("Used() inccorrect state = %v", n)
				}
			}
		})
	}
...
func Test_Int_Put(t *testing.T) {
	for _, tt := range uniquerandIntTestCases {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < tt.n; i++ {
				if gotOk := uri.Used(i); gotOk {
					t.Errorf("Used() invalid unique number = %v", i)
				}
			...
			for i := 0; i < tt.n; i++ {
				if gotOk := uri.Used(i); !gotOk {
					t.Errorf("Used() unexpected unique number = %v", i)
				}
				if gotOk := uri.Put(i); !gotOk {
			...
			// the count must be 0
			if uri.Count() != 0 {
				t.Errorf("Count() = %v != 0", uri.Count())
			}

			// putting any number outside the range should return false
			if gotOk := uri.Used(tt.n + 1); gotOk {
				t.Errorf("Used() out of range unique number = %v", tt.n+1)
			}
			if gotOk := uri.Put(tt.n + 1); gotOk {
				t.Errorf("Put() out of range unique number = %v", tt.n+1)
			}
